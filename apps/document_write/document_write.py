...
sample_query="请启动一个子任务智能体，打印hello world。"
sample_query="根据用户所提供的预警信息，生成一份卫星健康监测周报。"
...
#### 3. **路径必须双引号**：例如 `"./document_write/skills/..."`
#### 4. **禁止裸 `;`**：用 `&&` 连接命令
...
def run_bash(command: str) -> str:
    ...
    try:
        if os.name == "nt":
            ...
            if not args:
                return "Error: Empty command"
            if args[:2] == ["python", "-c"] and len(args) >= 3 and "\n" in args[2]:
                return "ValueError('禁止多行 python -c')"
...
def memory_search(query:str)->list:
    try:
        ...
        memory_prompt=f"""你是一个记忆检索大师，根据用户的提问，从已有的记忆文件里面检索相关度在80%以上的记忆文件，并且按照相关性排序，选择前5条记忆文件。
...
def run_edit(path: str, old_text: str, new_text: str) -> str:
    try:
        ...
        for enc_try in [enc, "utf-8", "gbk", "gb2312", "latin-1"]:
            try:
                content = fp.read_text(encoding=enc_try)
...
def run_subagent(prompt: str) -> str:
    ...
    for u in range(30):  # safety limit
        sub_content = ""
        sub_tool = {}
        id_print = 0
        name_print = 0
            ...
            if chunk.choices:
                ...
                if delta.tool_calls:
                    for tool_chunk in delta.tool_calls:
                        ...
                        # 初始化收集器
                        if idx not in sub_tool:
                            sub_tool[idx] = {"id": "", "type": "function", "function": {"name": "", "arguments": ""}}
                        # 收集id
                        if tool_chunk.id:
                            sub_tool[idx]["id"] += tool_chunk.id
                            if not id_print:
                                print(f"\n本次工具调用的id是:")
                                id_print = 1
                            print(tool_chunk.id, end="", flush=True)
                        if tool_chunk.function and tool_chunk.function.name:
                            sub_tool[idx]["function"]["name"] += tool_chunk.function.name
                            if not name_print:
                                print(f"\n本次工具调用的名称是:")
                                name_print = 1
                            print(tool_chunk.function.name, end="", flush=True)
                        if tool_chunk.function and tool_chunk.function.arguments:
                            sub_tool[idx]["function"]["arguments"] += tool_chunk.function.arguments
                            if not arg_print:
                                print(f"\n本次工具调用的参数为:")
                                arg_print = 1
                            print(tool_chunk.function.arguments, end="", flush=True)
        print(f"\n------------------△△△△△本次子智能体的输出结束△△△△△--------------------------\n")
        sub_assistant_message = {"role": "assistant", "content": sub_content if sub_content else None}
        if sub_tool:
            sub_assistant_message["tool_calls"] = [
                {
                    "id": data["id"],
                    "type": data["type"],
                    "function": {
                ...
                } for idx, data in sorted(sub_tool.items()) if data["id"]
            ]

        sub_context.append(sub_assistant_message)
        if not sub_assistant_message.get("tool_calls"):
            break

        results = []
        for block in sub_assistant_message.get("tool_calls"):
            if block.get("type") == "function":
                json_pr=json.loads(block.get("function").get("arguments"))
                ...
                print(f"--------子任务智能体的第{u}次工具调用结果为-----------\n{str(output[:500])}\n---------------工具输出结束-------------\n")
                #results.append({"type": "tool_result", "tool_use_id": block.id, "content": str(output)[:5000]})
            sub_context.append({"role":"tool","tool_call_id":block.get("id"),"content":json.dumps(output)})

        #print(f"------------------子任务智能体调用结束，最终回复为------------------\n")
        #print(response.choices[0].message.content)
        #print(f"------------------子任务智能体调用结束，最终回复结束------------------\n")
        #sub_messages.append({"role": "user", "content": results})
    # Only the final text returns to the parent -- child context is discarded
    print("\n------------------子任务智能体调用结束，最终回复为------------------\n")
    print(f"子智能体运行结束，最终回复内容为：{sub_assistant_message}")
    print("\n------------------子任务智能体调用结束，最终回复结束------------------\n")
    return f"子智能体运行结束，最终回复内容为：{sub_assistant_message}" or "(no summary)"
...
Tools=[
    {"type":"function",
     "function":{
         "name":"bash","description":"运行一个shell command。需要注意，请写好stdout，确保智能体了解程序运行结果。",
         "parameters":{"type":"object","properties":{
             "command":{"type":"string","description":"要运行的命令。需要注意，必须要写好stdout，确保智能体了解程序运行结果。除非程序运行失败，否则必然要有stdout输出"}
         }}
     }},
    {"type": "function",
     "function": {
    ...
    {"type": "function",
                 "function": {
                     "name": "plot_figure", "description": "绘制时序数据图表，x_data为时间点列表，y_data为对应的值列表，y_label为y轴的标签",
                     "parameters": {"type": "object", "properties": {
                         "x_data": {"type": "array",
    ...
    while True:
        ...
        for block in assistant_message.get("tool_calls"):
            if block.get("type") == "function":
                ...
                #print(f"{argument_json}")
                tool_name=block.get("function").get("name")
                if tool_name=="task":
                    description=argument_json.get("description","subagent")
                    user_prompt=argument_json.get("prompt")
                    print(f"------------△△△△△△△启动子任务智能体：△△△△△△△---------")
                    print(f"子任务智能体的描述为：{description}")
                    output = run_subagent(user_prompt)
                else:
                    handler = TOOL_HANDLERS.get(block.get("function").get("name"))
                    #argument_json = json.loads(block.get("function").get("arguments"))
                    try:
...
if __name__=="__main__":
    ...
    while user_turn:
        ...
        try:
            ...
            for fault_file in fault_file_list:
                with open(os.path.join(memory_fault_path,fault_file),"r",encoding="utf-8") as f_fault:
                    fault=f_fault.read()
                    history.append({"role":"assistant","content":fault})
        except Exception as e:
        ...
        #将history保存成为文件
        conservation_path=os.path.join(os.getcwd(),r"history.md")
        with open(conservation_path,"w",encoding="utf-8") as f:
            for item in history:
                f.write(str(item)+"\n")
